 ...
 */
(function($) {$.widget('ui.multiOpenAccordion', {
    options : {
      active : 0,
      showAll : null,
      hideAll : null,
      _classes : {
        ...
        h1 : 'ui-accordion-header ui-helper-reset ui-state-default ui-corner-all',
    ...
    _create : function() {
      var self = this, options = self.options, $this = self.element, $h1 = $this.children('h1'), $div = $this.children('div');
      $this.addClass(options._classes.accordion);
      $h1.each(function(index) {
        var $this = $(this);
        $this.addClass(options._classes.h1).prepend('<span class="{class}"></span>'.replace(/{class}/, options._classes.span));
        if(self._isActive(index)) {self._showTab($this)
        }
      });
      ...
      $h1.bind('click', function(e) {e.preventDefault();
        var $this = $(this);
        var ui = {
          tab : $this,
        ...
        if($this.hasClass(options._classes.stateDefault)) {self._showTab($this);
        } else {self._hideTab($this);
        }
      });
      $h1.bind('mouseover', function() {$(this).addClass(options._classes.stateHover);
      });
      $h1.bind('mouseout', function() {$(this).removeClass(options._classes.stateHover);
      });
    ...
    destroy : function() {
      ...
      $this.children('h1').unbind('click mouseover mouseout');
      $this.removeClass(options._classes.accordion);
      $h1.removeClass(options._classes.h1).removeClass('ui-state-default ui-corner-all ui-state-active ui-corner-top').children('span').remove();
    ...
    _hideTab : function($this) {console.log('hideTab')
      var $span = $this.children('span.ui-icon');
      var $div = $this.next();
      var options = this.options;
    ...
    _setActiveTabs : function(tabs) {
      var self = this;
      var $this = this.element;
      if( typeof tabs != 'undefined') {$this.children('div').each(function(index) {
          var $tab = $(this).prev('h1');
          if(tabs.hasObject(index)) {self._showTab($tab);
          } else {self._hideTab($tab);
          }
        });
      }
    },
    _generateTabsArrayFromOptions : function(tabOption) {
      ...
      var self = this;
      var $this = self.element;
      var size = $this.children('h1').size();
      if($.type(tabOption) === 'array') {
        ...
        switch(tabOption.toLowerCase()) {
          case'all':
            var size = $this.children('h1').size();
            for(var n = 0; n < size; n++) {tabs.push(n);
            }
            return tabs;
            break;
          case'none':
